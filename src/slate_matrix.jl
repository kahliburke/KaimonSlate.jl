...
const _MATRIX_BANDED_TYPES = Union{
...
function _matrix_heatmap(
    ...
    nr, nc = size(z)
    w, h = if nc >= nr
        (base_px, round(Int, base_px * nr / nc))
    else
        (round(Int, base_px * nc / nr), base_px)
    end
    return echart(;
        xAxis=(type="category", data=string.(1:gnc), show=false, splitArea=(show=false,)),
        yAxis=(
            type="category", data=string.(1:gnr), show=false, splitArea=(show=false,), inverse=true
        ),
...
function _matrix_texnum(x::Complex, digits)
    ...
    return i == 0 ? string(r) : string(r, i < 0 ? "-" : "+", abs(i), "i")
end
...
function Base.show(io::IO, ::MIME"text/latex", t::BlockMatrixTeX)
    ...
    return print(io, "\\end{array}\\right]\$\$")
end
...
function slate_matrix(
    ...
    Mv = if (rows === nothing && cols === nothing)
        M
    else
    ...
    if k === :katex
        return if blockrows === nothing
            MatrixTeX(Mv, digits)
        else
            BlockMatrixTeX(Mv, blockrows, blockcols, digits)
        end
    elseif k === :dotted
        d =
            if Mv isa Union{
                ...
                first(LinearAlgebra.diag(Mv))
            else
                Mv[1, 1]
            end
        o = if Mv isa LinearAlgebra.SymTridiagonal
            (isempty(Mv.ev) ? 0.0 : first(Mv.ev))
        elseif Mv isa LinearAlgebra.Tridiagonal
            (isempty(Mv.du) ? 0.0 : first(Mv.du))
        else
            0.0
        end
        return DottedMatrixTeX(
            max(size(Mv)...), round(real(d); digits=digits), round(real(o); digits=digits)
        )
    else

...
(function () {
  ...
  window.__healthRestart = async function () {
    if (!confirm('Restart the worker?\n\nThis clears the namespace — every value recomputes on the next run. Use it when a kernel is wedged (runaway / unreachable) and a graceful stop can\'t reach it.')) return;
    try { await api('POST', '/api/restart'); poll(); } catch (_) {}
  };
  ...
  function alertRow(al) {
    ...
    const act = isCell
      ...
      : `<button class="hpact danger" onclick="event.stopPropagation();window.__healthRestart()">Restart worker</button>`;

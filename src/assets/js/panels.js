...
function connectLive() {
  ...
  es.onmessage = async (e) => {
    ...
    if (e.data.startsWith('runbatch:')) { const _rb = e.data.slice(9).split(':'); window.onRunBatch && window.onRunBatch(parseInt(_rb[0], 10) || 0, _rb[1] === '1'); return; }   // N cells about to run (+ whether it STARTS a run) → stable k/N
    ...
    if (e.data.startsWith('inspectmany:')) { try { const r = JSON.parse(e.data.slice(12)); window._slateInspectMany && window._slateInspectMany(r.reqid, r.cells); } catch (_) {} return; }   // PDF export: every HTML-output cell, one round-trip

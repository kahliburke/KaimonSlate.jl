...
function _celldigest(cells)
    return [
        Dict("id" => string(id), "kind" => string(kind), "hash" => _sha(src)) for
        (id, kind, src) in cells
    ]
end
    ...
    for e in es
        if haskey(e, "cells")            # legacy full snapshot → replaces state wholesale
            ...
            for c in get(e, "chg", Any[])
                id = string(c["id"])
                haskey(by, id) || push!(order, id)
                by[id] = Dict{String,Any}(
                    "id" => id,
                    "kind" => string(get(c, "kind", "code")),
                    "hash" => string(c["hash"]),
                )
            end
...
function _prime!(key, path)
    haskey(_TAIL, key) && return nothing
    d = _dir(path)
    es = entries(path)
    if isempty(es)
        _TAIL[key] = (0, "", Any[])
    ...
    full = if isfile(hp)
        try
            Vector{Any}(JSON.parse(read(hp, String)))
        catch
            Any[]
        end
...
function _derive_label(prev_cells, cur)
    ...
    for r in removed
        ...
        i === nothing && continue
        push!(renames, (r, avail[i]))
        deleteat!(avail, i)
    end
    if !isempty(renames)
        newids = Set(a for (_, a) in renames)
        oldids = Set(r for (r, _) in renames)
    ...
    parts = String[]
    isempty(renames) || push!(
        parts,
        if length(renames) <= 3
            join(("renamed $r → $a" for (r, a) in renames), ", ")
        else
            "renamed $(length(renames)) cells"
        end,
    )
...
)   # explicit action label; overrides the source-diff derivation
    h = _sha(source)
    key = _key(path)
    lock(_LOCK) do
        _prime!(key, path)
        ...
        open(_logpath(d), "a") do io
            return println(io, JSON.json(entry))
        end
...
function _migrate_dir!(keydir; sink)
    lp = _logpath(keydir)
    isfile(lp) || return nothing
    es = Dict{String,Any}[]
    for l in eachline(lp)
        isempty(strip(l)) && continue
        try
            push!(es, JSON.parse(l))
        catch
        end
    end
...
function migrate!(; verbose::Bool=true)
    ...
    for keydir in readdir(root; join=true)
        ...
        try
            _migrate_dir!(keydir; sink=sink)
        catch e
            verbose &&
                @warn "SlateHistory.migrate!: skipped $keydir" exception = (e, catch_backtrace())
        end
    end
    lock(_LOCK) do ;
        return empty!(_TAIL)
    end     # force re-prime under the new format
    write(joinpath(root, ".migrated_v2"), string(time()))
    verbose && @info "SlateHistory.migrate! done" dirs = sink[:dirs] log_MB = round(
        (sink[:log_before] - sink[:log_after]) / 1e6; digits=1
    ) obj_MB = round((sink[:obj_before] - sink[:obj_after]) / 1e6; digits=1)
    return sink
end

# Run migrate! once per cache root (marker-gated), at server startup.
function migrate_once!(; verbose::Bool=true)
    isfile(joinpath(_root(), ".migrated_v2")) && return nothing

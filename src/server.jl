...
using Base64: Base64
using Dates: Dates                                  # publish dates for the multi-doc site manifest
using Logging: Logging                                # standalone serve: route hub log detail to a file
using REPL: REPL                                   # standalone serve: raw-mode ^C byte (see _wait_for_ctrl_c)
using Tar: Tar
...
mutable struct LiveNotebook
    ...
    function LiveNotebook(
        ...
        return new(
            ...
            agents,
            Cell[],
            @NamedTuple{id::String, js::String, esm::Bool, kind::String}[],
            Dict{String,String}(),
        )
    end
end
...
const _KERNEL_BOUNDARY = Set{Symbol}((
    :prepare!,
    :eval_cell!,
    :eval_stale!,
    :_eval!,
...
function _find_boundary_call(ex)
    ...
    if ex.head === :macrocall
        m = ex.args[1]
        (m === Symbol("@async") || m === Symbol("@spawn") || m === Symbol("@spawnat")) &&
            return nothing
    end
    if ex.head === :call
        f = ex.args[1]
...
macro report_op(nb, report, body)
    ...
    bad === nothing || error(
        "@report_op: `$bad` is a kernel round-trip / blocking call and must not run while nb.lock " *
...
function _wire_callbacks!(nb::LiveNotebook)
    ...
    register_runbatch!(nb.report.id, n -> (
        try
            _broadcast(nb, "runbatch:$n")
        catch
        end
    ))   # run size → stable k/N
    register_userprog!(
        ...
        (frac, msg, id, done) -> (
            try
                _broadcast(
                    nb,
                    "cellprog:" *
                    JSON.json(Dict("frac" => frac, "msg" => msg, "id" => id, "done" => done)),
                )
            catch
            end
        ),
    )
    register_prepare!(nb.report.id, json -> (
        try
            _broadcast(nb, "prepare:" * json)
        catch
        end
    ))   # env precompile progress → "Preparing packages" banner
    register_emit!(nb.report.id, (channel, payload) -> (
        try
            _ws_emit!(nb, channel, payload)
        catch
        end
    ))   # slate_emit → push over the page WebSocket (NOT the coalescing SSE); payload is a Julia value, JSON-encoded in _ws_emit!
    register_bin_emit!(nb.report.id, frame -> (
        try
            _ws_broadcast_bin!(nb, frame)
        catch
        end
    ))   # slate_emit_bin → forward the raw binary frame over the page WebSocket as-is
    register_celldone!(nb.report.id, (run_id, cid, wire) -> server_celldone(nb, run_id, cid, wire))   # parallel-batch result merge
    register_cleanup_cells!(nb.report.id, ids -> (
        try
            _cleanup_deleted_cells(nb, ids)
        catch
        end
    ))   # deleted-cell slate_on_cleanup teardown
    return nb
end
function _unwire_callbacks!(nb::LiveNotebook)
    unregister_refresh!(nb.report.id)
    unregister_srcchange!(nb.report.id)
    unregister_progress!(nb.report.id)
    unregister_runbatch!(nb.report.id)
    unregister_userprog!(nb.report.id)
    unregister_emit!(nb.report.id)
    unregister_celldone!(nb.report.id)
    unregister_prepare!(nb.report.id)
    unregister_bin_emit!(nb.report.id)
    unregister_cleanup_cells!(nb.report.id)
    return nb
end
...
function _cleanup_deleted_cells(nb::LiveNotebook, ids)
    ...
    @async try
        ...
        for k in regionks
            try
                ReportEngine.run_cleanups!(k, nb.report, sids)
            catch
            end
        end
    catch e
...
function _instantiate_env!(
    envdir::AbstractString;
    online=nothing,
    code::AbstractString="using Pkg; Pkg.instantiate()",
    quiet::Bool=true,
)
    jl = Base.julia_cmd()[1]
    ok = true
...
function _rebuild_notebook_env!(envdir::AbstractString, parent::AbstractString; online=nothing)
    ...
    if !build()
        ...
        for f in ("Project.toml", "Manifest.toml")
            try
                rm(joinpath(envdir, f); force=true)
            catch
            end
        end
        build()
...
function _select_kernel(path::AbstractString, report; threads::AbstractString="", online=nothing)
    ...
    if ReportEngine.gate_available()
        ReportEngine._rlog(
            "_select_kernel nb=$(basename(String(path))) runon=[$(get(report.meta, "runon", ""))] remoteworker=[$(get(report.meta, "remoteworker", ""))]",
        )
        ...
        if !isempty(rw)
            ...
            (port !== nothing && sp !== nothing) && return ReportEngine.attach_gate_kernel(port, sp)
            @warn "slate: ignoring malformed remoteworker spec (want \"port,stream_port\")" spec =
                rw
        end
        ...
        if !isempty(ro)
            ...
            transport =
                length(parts) >= 2 && !isempty(strip(parts[2])) ? Symbol(strip(parts[2])) : :tunnel
            ...
            origin_env = if isfile(joinpath(envdir, "Project.toml"))
                envdir
            else
            ...
            rproj =
                "~/.cache/kaimonslate/remote/" * ReportEngine._remote_env_key(origin_env, parent)
            target = ReportEngine.RemoteTarget(
                rhost;
                transport=transport,
                project=rproj,
                port=pport,
            ...
            ReportEngine._rlog(
                "_select_kernel → REMOTE kernel host=$rhost transport=$transport rproj=$rproj parent=$parent",
            )
            ...
            return ReportEngine.GateKernel(
                rproj; parent=parent, threads=threads, target=target, label=basename(abspath(path))
            )
        end
        proj = Base.current_project(dirname(abspath(path)))
        parent = proj === nothing ? "" : dirname(proj)
        ...
        elseif env_exists
            ...
            if ReportEngine.env_stale(envdir, parent)
                ReportEngine._rlog(
                    "_select_kernel: notebook env stale vs parent → rebuilding $(basename(envdir))"
                )
...
function _boot_and_run!(nb::LiveNotebook; autorun::Bool=true)
    ...
    @async begin
        try
            ...
            kernel = _select_kernel(path, r; online=line -> (
                try
                    _broadcast(nb, "bringup:" * line)
                catch
                end
            ))
            lock(nb.lock) do ;
            ...
            if autorun
                ...
                end   # boot done → falls back to the (bannerless) "run" default
                try
                    _broadcast(nb, string(nb.version))
                catch
                end   # worker is up → refresh the dot to "connected" BEFORE the (possibly long) run, so it's not stale
            ...
            pending isa PendingKernel &&
                pending.real === nothing &&
                pending.err === nothing &&
...
function _hydrate_standalone!(nb::LiveNotebook, path::AbstractString)
    ...
    lock(nb.lock) do
        ...
        return nb.report.meta["hydratingKind"] = "env"
    end
    ...
    online =
        line -> begin
            ...
            startswith(s, "@@SLATE_PREP") || (
                try
                    _broadcast(nb, "bringup:" * first(s, 200))
                catch
                end
            )
            return nothing
    ...
    try
        ...
        try
            ...
            if packed !== nothing
                ...
                n > 0 &&
                    ReportEngine._rlog("hydrate: unpacked $n memo files from the standalone bundle")
            end
        catch e
            @warn "KaimonSlate: embedded memo unpack failed (cells will recompute)" exception = (
                e, catch_backtrace()
            )
        end
        kernel = GateKernel(
        ...
        lock(nb.lock) do
            ...
            return delete!(nb.report.meta, "preview")       # live cells supersede the frozen render
        end
        pending isa PendingKernel && ReportEngine._resolve!(pending, kernel)   # unblock anyone who raced the boot window
...
function _reactive_refresh!(nb::LiveNotebook, seed_predicate)
    ...
    lock(nb.lock) do
        ...
        cells = [
            cell_json(
                c, bindref, hostednames; nbid=nb.id, bibctx=bibctx, figidx=figidx, report=nb.report
            ) for c in nb.report.cells if c.id in changed
        ]
        return msg = "refresh:" * JSON.json(Dict("cells" => cells))
    end
...
function server_asset_changed(nb::LiveNotebook, changed::Vector{String})
    ...
    _resolve(rel) =
        isabspath(rel) ? String(rel) : (isempty(base) ? String(rel) : joinpath(base, rel))
...
worker_generation(nb::LiveNotebook) = lock(_MAIN_GEN_LOCK) do ;
    return get(_WORKER_GEN, nb.id, 0)
end
function _bump_worker_generation!(nb::LiveNotebook)
    lock(_MAIN_GEN_LOCK) do
        return _WORKER_GEN[nb.id] = get(_WORKER_GEN, nb.id, 0) + 1
    end
end
...
_eval_mutex(nb::LiveNotebook) = lock(_EVAL_MUTEX_LOCK) do ;
    return get!(() -> ReentrantLock(), _EVAL_MUTEX, nb.id)
end
...
function _self_heal_locked!(nb::LiveNotebook)
    ...
    targets = lock(nb.lock) do
        return [
            c for c in nb.report.cells if c.kind == CODE &&
                c.state == STALE &&
                :locked in c.flags &&
                !isempty(ReportEngine._locked_key(c))
        ]
    end
    for c in targets
        try
            lock(_eval_mutex(nb)) do ;
                return _eval_one!(nb, c)
            end
        catch e
            @warn "KaimonSlate: locked-cell self-heal failed" cell = c.id notebook = nb.id exception =
                e
        end
    end
    return nothing
...
function _upstream_closure(report, id::String)
    ...
    queue = String[id]
    while !isempty(queue)
        c = get(report.byid, popfirst!(queue), nothing)
        c === nothing && continue
        for d in c.deps
            d in seen && continue
            push!(seen, d)
...
function _stats_record!(nb::LiveNotebook, cell)
    ...
    lock(_CELL_STATS_LOCK) do
        ...
        s.last_ts = time()
        s.last_ms = out.duration_ms
        s.last_memo = out.memo
        if out.memo == "restored"
            s.restores += 1
        else
            s.evals += 1
            s.total_ms += out.duration_ms
            s.sumsq += out.duration_ms^2
            s.min_ms = min(s.min_ms, out.duration_ms)
            s.max_ms = max(s.max_ms, out.duration_ms)
...
function _cell_stats_json(nbid::AbstractString, cid::AbstractString)
    lock(_CELL_STATS_LOCK) do
        ...
        d = Dict{String,Any}(
            ...
            "restores" => s.restores,
            "pulls" => s.pulls,
            "total_ms" => r1(s.total_ms),
            "mean_ms" => r1(mean),
            "std_ms" => r1(sd),
            "min_ms" => n > 0 ? r1(s.min_ms) : 0.0,
            "max_ms" => r1(s.max_ms),
            "p50_ms" => r1(pct(0.5)),
            "p90_ms" => r1(pct(0.9)),
            "last_ms" => r1(s.last_ms),
            "last_ts" => r1(s.last_ts),
            "memo" => s.last_memo,
            "recent" => [r1(x) for x in s.recent],
        )   # the raw ring — the stats card's sparkline
...
function _stats_ran_on!(nb::LiveNotebook, cid::AbstractString, where::AbstractString)
    lock(_CELL_STATS_LOCK) do
        return get!(CellStats, get!(Dict{String,CellStats}, _CELL_STATS, nb.id), String(cid)).ran_on = String(
            where
        )
    end
    return nothing
...
function _stats_xfer!(nb::LiveNotebook, cid::AbstractString, desc::AbstractString, bytes::Integer)
    lock(_CELL_STATS_LOCK) do
        ...
        return s.last_xfer = String(desc)
    end
    return nothing
end
...
function _nb_defined_regions(nb::LiveNotebook)
    return String[
        String(get(d, "name", "")) for d in _regions_json(nb) if get(d, "defined", false) === true
    ]
end

_mesh_pending(nbid) = lock(_MESH_CONSENT_LOCK) do ;
    return get(_MESH_PENDING, String(nbid), nothing)
end
...
_mesh_resolve!(nbid) = lock(_MESH_CONSENT_LOCK) do
    delete!(_MESH_PENDING, String(nbid))
    return delete!(_MESH_DISMISSED, String(nbid))
end
function _mesh_dismiss!(nb::LiveNotebook)
    sig = _mesh_group_sig(_nb_defined_regions(nb))
    lock(_MESH_CONSENT_LOCK) do ;
        _MESH_DISMISSED[nb.id] = sig
        return delete!(_MESH_PENDING, nb.id)
    end
    return nothing
end
...
function _mesh_broadcast_clear!(nb::LiveNotebook)
    return _broadcast(nb, "mesh-consent:" * JSON.json(Dict("connected" => true, "pairs" => Any[])))
end
...
function _mesh_consent_check!(nb::LiveNotebook)
    ...
    @async try
        ...
        if length(hosts) < 2
            _mesh_resolve!(nb.id)
            return nothing          # no cross-host pair — clear any stale pending/dismissal
        end
        status = ReportEngine.mesh_consent_status(names)
        if status["connected"]
            _mesh_resolve!(nb.id)
            return nothing          # episode over — a fresh disconnect later re-prompts
        end
...
function set_notebook_regions!(nb::LiveNotebook, csv::AbstractString)
    ...
    lock(nb.lock) do
        ...
        return _persist!(nb; label=isempty(joined) ? "cleared regions" : "regions · $joined")
    end
...
function _side_kernel!(nb::LiveNotebook, side::AbstractString)
    return isempty(side) ? nb.kernel : _region_kernel!(nb, String(side))
end

# Tolerant field read of a harvested effect record — a NamedTuple locally, tolerant of a Dict/JSON3 shape.
function _effect_field(e, f::Symbol)
    return if e isa AbstractDict
        get(e, f, get(e, String(f), nothing))
    else
...
function _effect_record(e)
    ...
    kind = kind isa AbstractString ? Symbol(kind) : kind
    names = _effect_field(e, :names)
    names = names === nothing ? Symbol[] : Symbol[Symbol(n) for n in names]
    src = _effect_field(e, :stmt_src)
    src = src === nothing ? "" : String(src)
...
function _apply_cell_effects!(nb::LiveNotebook, c::Cell, out)
    ...
    for r in recs
        ...
        elseif r.kind !== nothing
            ReportEngine._rlog(
                "cell effects: cell $(c.id) declared unhandled effect kind ':$(r.kind)' — ignored"
            )
        end
    end
    ...
    try
        EffectStore.store!(SlateHome.effects_dir(), string(c.src_hash), recs)
    catch e
        ReportEngine._rlog(
            "cell effects: persist for $(c.id) failed: $(first(sprint(showerror, e), 120))"
        )
    end
    return nothing
end
...
function _reestablish_effects!(report)
    ...
    for c in report.cells
        ...
        recs = try
            EffectStore.load(root, string(c.src_hash))
        catch
            nothing
        end
...
function _manifest_field(e, f::Symbol)
    return if e isa AbstractDict
        get(e, f, get(e, String(f), nothing))
    else
        (hasproperty(e, f) ? getproperty(e, f) : nothing)
...
function _register_frontend!(
    nb::LiveNotebook, idv, js::AbstractString, esm::Bool=false, kind::AbstractString=""
)
...
function _refresh_extensions!(nb::LiveNotebook)
    manifest = try
        if nb.kernel isa ReportEngine.GateKernel
        ...
        ReportEngine._rlog(
            "slate: extension manifest refresh failed: $(first(sprint(showerror, e), 120))"
        )
        return false
    end
    ...
    if fe !== nothing
        for e in fe
            js = _manifest_field(e, :js)
            js === nothing && continue
            esm = _manifest_field(e, :esm)
            esm = esm === true || esm == "true"
            kind = _manifest_field(e, :kind)
            kind = kind === nothing ? "" : String(kind)
            lock(nb.lock) do
                return _register_frontend!(nb, _manifest_field(e, :id), String(js), esm, kind)
            end && (changed = true)
        end
    end
    ...
    if as !== nothing
        for e in as
            pkg = _manifest_field(e, :pkg)
            dir = _manifest_field(e, :dir)
            (pkg === nothing || dir === nothing) && continue
            lock(nb.lock) do
                return _register_assets!(nb, String(pkg), String(dir))
            end && (changed = true)
        end
    end
    return changed
...
function _frontend_scripts_json(nb::LiveNotebook)
    return [
        Dict{String,Any}("id" => e.id, "js" => e.js, "esm" => e.esm, "kind" => e.kind) for
        e in nb.frontend
    ]
end
...
function _region_route(nb::LiveNotebook, cell::Cell)
    ...
    (!isempty(side) && isempty(_cell_region(cell))) && ReportEngine._rlog(
        "region: cell $(cell.id) auto-follows its mutation target to region '$side' — a mutation runs where the value lives",
    )
...
function _region_kernel!(nb::LiveNotebook, name::String)
    lock(_REGION_LOCK) do
        ...
        r = ReportEngine.region_get(name)
        r === nothing && error(
            "region '$name' is not defined — create it in the registry: " *
        ...
        ReportEngine._rlog(
            "region: kernel '$name' for $(nb.id) → $(r.host) ($(r.transport))" *
            (isempty(r.data_root) ? "" : " root=$(r.data_root)"),
        )
        k = ReportEngine.GateKernel(
            target.project;
            parent=parent,
            target=target,
...
function _prime_namespace!(nb::LiveNotebook, k, side::AbstractString)
    ...
    lock(_REGION_LOCK) do ;
        return get(_REGION_PRIMED, key, UInt(0))
    ...
    lock(_REGION_LOCK) do
        return key in _REGION_PRIMING ? true : (push!(_REGION_PRIMING, key); false)
    end && return nothing
    try
        for c in env
            ...
            try
                _region_presync!(nb, c, k; dst_side=side)
            catch e
                ReportEngine._rlog(
                    "region: presync for prime of $(c.id) on $(_side_label(nb, side)) failed: " *
            ...
            try
                ReportEngine.eval_capture(
                    k, nb.report, prime_src, "cell:" * c.id * "#prime", nothing
                )
            catch e
                ...
                if prime_src != c.source
                    try
                        ReportEngine.eval_capture(
                            k, nb.report, c.source, "cell:" * c.id * "#prime", nothing
                        )
                    catch e2
                        ReportEngine._rlog(
                            "region: namespace prime of $(c.id) on $(_side_label(nb, side)) failed: " *
        ...
        lock(_REGION_LOCK) do ;
            return delete!(_REGION_PRIMING, key)
        end
    end
    lock(_REGION_LOCK) do ;
        return _REGION_PRIMED[key] = sig
    end
    return nothing
end
...
function _everywhere_replay_source(c::Cell)
    ...
    scaffold = if ReportEngine._is_pure_using(c.source)
        c.source
    else
        ReportEngine._scaffold_replay_source(c.source)
    end
    (:everywhere_declared in c.flags) || return isempty(scaffold) ? c.source : scaffold
    recs = if (c.output !== nothing && !isempty(c.output.effects))
        c.output.effects
    else
        try
            EffectStore.load(SlateHome.effects_dir(), string(c.src_hash))
        catch
            nothing
        end
    end
    ...
    return isempty(stmts) ? c.source : join(stmts, "\n")
end
...
function _teardown_region!(nb::LiveNotebook; kill::Bool=false)
    ks = lock(_REGION_LOCK) do
        got = [
            pop!(_REGION_KERNELS, key) for key in collect(keys(_REGION_KERNELS)) if key[1] == nb.id
        ]
    ...
    for k in ks
        try
            ReportEngine.shutdown!(k; kill_remote=kill)
        catch e
...
function restart_region!(nb::LiveNotebook, side::AbstractString)
    ...
    k === nothing || try
        ReportEngine.shutdown!(k; kill_remote=true)
    catch e
    ...
    ReportEngine._rlog(
        "region: restart '$side' for $(nb.id) — killed worker, restaled $(length(ids)) cell(s)"
    )
    @async begin
        try
...
const _DEAD_WIRE_GRACE = something(
    tryparse(Float64, get(ENV, "KAIMONSLATE_DEADWIRE_GRACE", "")), 45.0
)  # s of continuous silence ⇒ dead
...
function _region_autoretry()
    return something(
        tryparse(Bool, get(ENV, "KAIMONSLATE_REGION_AUTORETRY", "")), _REGION_AUTORETRY[]
    )
end
...
function _heal_dead_wire!(nb::LiveNotebook, k, unresp_s::Real=0.0)
    ...
    ReportEngine._rlog(
        "liveness: dead wire on $(nb.id)/$(side) — worker unresponsive for $(round(Int, unresp_s))s (grace $(round(Int, _DEAD_WIRE_GRACE))s); dropping the connection ($(auto ? "auto-retry: next run re-dials" : "manual: holds until an explicit re-run"))",
    )
    dropped = try
        ReportEngine._drop_kernel_conn!(k)
    catch e
        ReportEngine._rlog(
            "liveness: drop failed on $(nb.id)/$(side): " * first(sprint(showerror, e), 120)
        )
        false
    end
    dropped && !auto && (
    ...
    )   # pill flips to amber "reconnecting" NOW, not at the next state
    return dropped
end
    ...
    for k in _nb_kernels(nb)
        ...
        try
            r = ReportEngine._tool(
                k, "__slate_running", Dict{String,Any}(); timeout=_LIVENESS_PING_TIMEOUT
            )
            run = if r isa NamedTuple
                get(r, :running, nothing)
            elseif r isa AbstractDict
                get(r, "running", get(r, :running, nothing))
            else
                nothing
            end
            if run !== nothing
            ...
            ReportEngine._rlog(
                "liveness: __slate_running failed on $(nb.id)/$(_kernel_side_label(nb, k)): " *
        ...
        if ok
            if haskey(_KERNEL_UNRESPONSIVE_SINCE, k)   # was unwell → recovered this sweep
                ...
                end    # pill back to green immediately
            end
...
function _drop_kernels_for_worker!(h, host::AbstractString, port::Integer)
    ...
    for nb in nbs, k in _nb_kernels(nb)
        ...
        try
            if ReportEngine._drop_kernel_conn!(k)
                n += 1
                ReportEngine._rlog(
                    "reap: dropped live wire on $(nb.id)/$(_kernel_side_label(nb, k)) (worker-$port on $host reaped)",
                )
                try
                    _workers_push!(nb)
                catch
                end   # pill flips to amber "reconnecting" immediately
            end
        catch
        end
    end
    ...
    n == 0 && ReportEngine._rlog(
        "reap: no live kernel matched $host:$port (live remote kernels: $(isempty(seen) ? "none" : join(seen, ", "))) — liveness sweep will backstop",
    )
    return n
end
...
function _reconcile_nb_runs!(nb::LiveNotebook)
    ...
    for c in running
        get!(_RUN_SINCE, (nb.id, c.id), now)
    end   # stamp first-seen-running
    ...
    for c in suspects
        ...
        idx = _index_of(nb.report.cells, c.id)
        idx === nothing && continue
        did = lock(nb.lock) do
            return ReportEngine.revert_running!(c)
        end
        did || continue
        delete!(_RUN_SINCE, key)
        delete!(_RUN_ORPHAN_HITS, key)
        ReportEngine._rlog(
            "supervisor: healed orphaned run — $(nb.id)/$(c.id) was RUNNING but no kernel is evaluating it → reset to stale",
        )
        try
            _announce_cell!(nb, idx)
        catch
        end
    end
    return nothing
...
function _reconcile_stale_runner!(nb::LiveNotebook)
    active = lock(_RUNNER_LOCK) do ;
        return get(_RUNNERS, nb.id, false)
    end
    ...
    has_pending, any_running = lock(nb.lock) do
        ...
        any(c -> c.state == RUNNING, nb.report.cells)
    end
    ...
    started = lock(_RUNNER_LOCK) do ;
        return get(_RUNNER_STARTED, nb.id, time())
    end
    ...
    lock(_RUNNER_LOCK) do
        ...
        return delete!(_RUNNER_CANCEL, nb.id)
    end
    ReportEngine._rlog(
        "supervisor: notebook $(nb.id) looked wedged — a runner was marked active for " *
        "$(round(Int, time() - started))s with pending work and nothing actually running. " *
        "Clearing the stale flag and restarting its runner.",
    )
    @warn "slate: self-healed a wedged notebook runner" notebook = nb.id stuck_for_s = round(
        Int, time() - started
    )
    _ensure_runner!(nb)
    return nothing
...
function _supervise_runs!(h)   # NOTE: `Hub` is defined later (server_hub.jl, included at ~1510) — untyped so this loads
    try
        ReportEngine.reap_pending_kills!()   # hub-wide, not per-notebook — see gate_kernel.jl
    catch e
        ReportEngine._rlog(
            "supervisor: pending-kill reap error: " * first(sprint(showerror, e), 120)
        )
    end
    nbs = lock(h.lock) do ;
        return collect(values(h.notebooks))
    ...
    for nb in nbs
        try
            _liveness_sweep!(nb)   # heartbeat + dead-wire heal; caches running ids for the reconciler
        catch e
            ReportEngine._rlog(
                "supervisor: liveness error on $(nb.id): " * first(sprint(showerror, e), 120)
            )
        end
        try
            _reconcile_nb_runs!(nb)
        catch e
            ReportEngine._rlog(
                "supervisor: reconcile error on $(nb.id): " * first(sprint(showerror, e), 120)
            )
        end
        try
            _reconcile_stale_runner!(nb)
        catch e
            ReportEngine._rlog(
                "supervisor: stale-runner reconcile error on $(nb.id): " *
                first(sprint(showerror, e), 120),
            )
        end
        try
            ...
            ReportEngine._rlog(
                "watchdog: scan error on $(nb.id): " * first(sprint(showerror, e), 120)
            )
        end
        try
            _ws_health!(nb)
        catch
        end   # push watchdog status to open pages over the WS (replaces the GET /api/health poll)
    end
    return nothing
end
...
nb_health(id::AbstractString) = lock(_WD_LOCK) do ;
    return get(_NB_HEALTH, String(id), nothing)
end
...
function _watchdog_scan!(nb::LiveNotebook)
    ...
    for (side, latest, hist) in _nb_kernel_stats(nb)
        ...
        if length(recent) >= _WD_CPU_SAMPLES &&
            all(s -> s.cpu >= _WD_CPU_HOT, recent) &&
            !isempty(latest.running)
            push!(
                alerts,
                (
                    kind="runaway-cpu",
    ...
    status = if isempty(alerts)
        "ok"
    elseif any(a -> a.kind in ("stalled", "runaway-mem", "unreachable"), alerts)
        "critical"
    else
        "warning"
    end
...
function _health_transition!(nb::LiveNotebook, rec)
    prev = lock(_WD_LOCK) do ;
        return get(_NB_HEALTH, nb.id, nothing)
    end
    prevkeys =
        prev === nothing ? Set{String}() : Set(string(a.kind, ':', a.target) for a in prev.alerts)
    newkeys = Set(string(a.kind, ':', a.target) for a in rec.alerts)
    for a in rec.alerts
        string(a.kind, ':', a.target) in prevkeys || ReportEngine._rlog(
            "watchdog: $(nb.id) $(a.kind) [$(a.scope) $(a.target)] — $(a.detail)"
        )
    end
    for k in prevkeys
...
function _hub_src_stale()
    return !isempty(_HUB_START_SHA[]) && (
        try
            ReportEngine._payload_sha() != _HUB_START_SHA[]
        catch
            false
        end
    )
...
function _health_json(nb::LiveNotebook)
    ...
    rec === nothing &&
        return Dict{String,Any}("status" => "ok", "alerts" => Any[], "src_stale" => stale)
    now = time()
...
function _xfer_plan_gate(nb::LiveNotebook, cell::Cell, host::AbstractString, name, token::String)
    return function (bytes::Int, meta, rate=nothing)
        ...
        approved = lock(_REGION_LOCK) do
            return if key in _XFER_OFFERED
                (delete!(_XFER_OFFERED, key); true)
            else
                (push!(_XFER_OFFERED, key); false)
            end
        end
        approved && return nothing
        return error(
            "this cell needs '$name' from the other kernel: $(round(Int, bytes / 2^20)) MB " *
...
function _ensure_resource_on!(
    nb::LiveNotebook, cell::Cell, dst_k, dst_side::AbstractString; seen::Set{UInt}=Set{UInt}()
)
    key = (nb.id, _worker_key(dst_k), cell.src_hash)
    lock(_REGION_LOCK) do ;
        return get(_REGION_RESOURCED, key, false)
    end && return nothing
    ...
    for r in cell.reads
        ...
        for o in nb.report.cells
            (
                o !== cell &&
                r in o.writes &&
                _cell_side(nb, o) != dst_side &&
                !ReportEngine._is_pure_using(o.source)
            ) && (writer=o; break)
        end
        ...
        if ReportEngine._cell_effect(writer) == ReportEngine.RESOURCE
            ...
            try
                ...
                ReportEngine._rlog(
                    "resource: staging '$r' for cell $(cell.id) on " *
    ...
    ReportEngine._rlog(
        "resource: opened $opened on $(_side_label(nb, dst_side)) — " *
        "replayed cell $(cell.id) (handle not shipped)",
    )
    lock(_REGION_LOCK) do ;
        return _REGION_RESOURCED[key] = true
    end
    return nothing
end
...
function _region_presync!(
    nb::LiveNotebook, cell::Cell, dst_k; dst_side::AbstractString=_cell_region(cell)
)
    ...
    for m in cell.mutates
        ...
        error(
            "cell mutates '$m', which lives on " *
            _side_label(nb, _cell_side(nb, owner)) *
            " (written by cell $(owner.id)) — but this cell runs on " *
            _side_label(nb, dst_side) *
    ...
    try
        _sync_datadir_to!(nb, dst_k; cell_id=cell.id)
    catch e
        @warn "slate region: datadir sync failed" cell = cell.id exception = e
    end
    prepared = false
    for r in cell.reads
        ...
        (r in writer.provides || ReportEngine._cell_effect(writer) == ReportEngine.EVERYWHERE) &&
            continue
        ...
        if ReportEngine._cell_effect(writer) == ReportEngine.RESOURCE
            if !prepared
                ReportEngine.prepare!(src_k, nb.report)
                ReportEngine.prepare!(dst_k, nb.report)
        ...
        seen = lock(_REGION_LOCK) do ;
            return get(get(_REGION_SYNCED, nb.id, Dict{String,String}()), key, "")
        end
        seen == token && continue
        ...
        port = try
            ReportEngine._tool(
                src_k, "__slate_portable", Dict{String,Any}("name" => string(r)); timeout=20.0
            )
        catch
            nothing
        end
        if port !== nothing && _gf(port, :portable, true) === false
            ptype = string(_gf(port, :type, "a handle"))
            preason = string(_gf(port, :reason, "a live handle"))
            error(
                "cell needs '$r' from " *
                _side_label(nb, src_side) *
                ", but it's a " *
                ptype *
                " — " *
                preason *
        ...
        whost = if src_k.target isa ReportEngine.RemoteTarget
            src_k.target.ssh_host
        elseif dst_k.target isa ReportEngine.RemoteTarget
        ...
        t0 = time()
        t = ReportEngine.transfer_binding!(
            src_k,
            dst_k,
            string(r);
            zc=zc,
            ...
            on_progress=onprog,
            cellkey=ReportEngine._memo_key(nb.report, writer),
        )   # lets the source restore from memo if its worker swapped
        secs = round(time() - t0; digits=1)
        ReportEngine._rlog(
            "region: '$(r)' → $(_side_label(nb, dst_side)) " *
            (
                if t.bytes == 0
                    "(deduped — already in the destination CAS via $(t.mode), 0 bytes moved) "
                else
        ...
        t.bytes > 0 && _stats_xfer!(
            nb,
            cell.id,
            "'$(r)' $(round(t.bytes / 2^20; digits = 1)) MB $(t.codec) in $(secs)s $arrow $whost",
            t.bytes,
        )
        lock(_REGION_LOCK) do
            return get!(_REGION_SYNCED, nb.id, Dict{String,String}())[key] = token
        end
    end
    return nothing
end
...
function _prepare_region_for_cell!(nb::LiveNotebook, cell::Cell, kernel, side::AbstractString)
    ...
    end   # pill appears NOW as "starting", not after the run completes
    try
        ...
        end   # connected → pill flips out of "starting"; telemetry takes over
        return true
    catch e
        ReportEngine._rlog(
            "region: prime before $(cell.id) on $host failed: " * first(sprint(showerror, e), 160)
        )
        ...
        lock(nb.lock) do
            ReportEngine.mark_errored!(
                cell, "region worker on $host could not start: " * first(sprint(showerror, e), 160)
            )
            return _broadcast_progress(nb, cell)
        end
        try
        ...
        end   # push the failure → pill goes amber/disconnected, not stuck "starting"
        return false
    end
end
...
function _eval_one!(nb::LiveNotebook, cell::Cell)
    ...
    if cell.kind == MARKDOWN
        ...
        try
            ...
            @warn "slate region: md presync failed" cell = cell.id exception = e
        end
        ...
        isempty(side) || lock(nb.lock) do ;
            return _broadcast_progress(nb, cell)
        end   # region md set RUNNING above → push the final state
        return nothing
    end
    ...
    src, srchash, memo, locked = lock(nb.lock) do
        ...
        frc = let ids = get(_FORCE_RUN, nb.id, nothing)   # consume a one-shot ▶ force marker
            if ids !== nothing && cell.id in ids
                (delete!(ids, cell.id); isempty(ids) && delete!(_FORCE_RUN, nb.id); true)
            else
                false
            end
        end
        ...
        unread = String[
            string(w) for w in defs if !any(o -> o !== cell && w in o.reads, nb.report.cells)
        ]
        ...
        safe = String[
            string(w) for w in defs if !any(o -> o !== cell && w in o.mutates, nb.report.cells)
        ]
        ...
        m = (
            ...
            names=unique!(
                String[
                    string(w) for w in Iterators.flatten((defs, cell.mutates)) if
                    w !== ReportEngine._THEME_SENTINEL && !(w in bindnames)
                ],
            ),
        ...
        return (s, cell.src_hash, m, locked)
    end
    out = try
        ...
        ReportEngine.eval_capture(
            ...
            memo;
            region=side,
            regions=_nb_region_names(nb),
        )
    catch e
        ReportEngine.CellOutput(
            "",
            ReportEngine.MimeChunk[],
            Any[],
            Any[],
    ...
    if _region_active(nb) && ReportEngine._is_pure_using(cell.source) && out.exception === nothing
        ...
        for c in nb.report.cells
            s = _cell_region(c)
            isempty(s) || push!(sides, s)
        end
        delete!(sides, side)
        for sd in sides
            try
                other = _side_kernel!(nb, sd)
                r2 = ReportEngine.eval_capture(
                    other, nb.report, src, "cell:" * cell.id * "#mirror", nothing
                )
                r2.exception === nothing || ReportEngine._rlog(
                    "region: `using` mirror of $(cell.id) failed on $(_side_label(nb, sd)): $(first(String(r2.exception), 200))",
                )
            catch e
                ReportEngine._rlog(
                    "region: `using` mirror of $(cell.id) errored on $(_side_label(nb, sd)): $(first(sprint(showerror, e), 200))",
                )
            end
        end
    end
...
function _emit_pending(nb::LiveNotebook, pending::Integer)
    return ReportEngine._emit_run_batch(nb.report.id, pending)
end
...
function set_runon_default!(spec::AbstractString)
    ...
    p === nothing || (
        try
            ...
            @warn "slate: could not persist run-location default" exception = e
        end
    )
...
function _cell_defines(cell::Cell)
    top = try
        ...
        return true
    end   # unparseable → conservative barrier
    ...
    for s in stmts
        ...
        s.head in (:function, :struct, :macro, :abstract, :primitive, :incomplete, :error) &&
            return true
...
function _preempt_victims(cells)
    return String[
        c.id for c in cells if c.kind == CODE &&
            c.state == RUNNING &&
            !_cell_defines(c) &&
...
function _preempt_superseded!(nb::LiveNotebook, cells)
    ...
    n > 0 && @info "slate: preempted superseded in-flight cells" notebook = nb.id cells = join(
        victims, ","
    )
    return nothing
end
...
function _batch_specs(code)
    ...
    for c in code
        ...
        push!(
            specs,
            ParCell(c.id, copy(c.deps), copy(c.reads), w, (:opaque in c.flags) || _cell_defines(c)),
        )
    end
    return specs
end
...
function _run_code_batch!(nb::LiveNotebook)
    ...
    specs, npending = lock(nb.lock) do
        ...
        return (ss, count(c -> c.state in (STALE, RUNNING), nb.report.cells))
    end
    ...
    try
        ...
        @warn "slate parallel: worker not ready — falling back to serial" notebook = nb.id exception =
            e
        return false
    end
    ...
    run_scheduled(
        ...
        function (id)
            cell = lock(nb.lock) do
                i = _index_of(nb.report.cells, id)
                return i === nothing ? nothing : nb.report.cells[i]
            end
            ...
            if get(_PARALLEL_CANCEL, nb.id, false)
                ...
                lock(nb.lock) do
                    (cell.state in (STALE, RUNNING)) || return nothing
                    ReportEngine.mark_errored!(cell, "InterruptException: run cancelled")
                    return _broadcast_progress(nb, cell)
                end
                return nothing
            end
            return _eval_one!(nb, cell)   # marks RUNNING + broadcasts, evals OFF-lock, merges version-guarded + broadcasts
        end,
    )
...
function server_celldone(nb::LiveNotebook, run_id::AbstractString, cid::AbstractString, wire)
    ...
    lock(nb.lock) do
        ...
        i === nothing && return nothing                          # deleted mid-batch → drop
        c = nb.report.cells[i]
        ...
        _apply_cell_effects!(nb, c, out)                 # code→Slate declarations (e.g. :everywhere classification)
        _stats_record!(nb, c)                            # before the broadcast — the push carries fresh stats
        return _broadcast_progress(nb, c)
    end
    return nothing
end
...
function _reestablish_fresh_namespace!(nb::LiveNotebook)
    ...
    try
        ...
        return nothing
    end   # up (bumps ns_gen if fresh)
    wk = _worker_key(nb.kernel)
    lock(_MAIN_GEN_LOCK) do ;
        return get(_MAIN_GEN, nb.id, UInt(0))
    end == wk && return nothing   # reattach → unchanged
    ...
    for (name, value) in binds                         # seed the fresh registry with the host's current values
        try
            ReportEngine.assign_bind!(nb.kernel, nb.report, name, value)
        catch e
            @debug "slate: bind re-seed failed on fresh namespace" name exception = e
        end
    end
    lock(_MAIN_GEN_LOCK) do ;
        return _MAIN_GEN[nb.id] = wk
    end
    return nothing
end
...
function _run_loop!(nb::LiveNotebook)
    try
        ...
        paths = lock(nb.lock) do ;
            return ReportEngine.unresolved_using_paths(nb.report)
        end
        if !isempty(paths) && ReportEngine.resolve_usings!(nb.report, nb.kernel, paths)
            lock(nb.lock) do ;
                return ReportEngine.rebuild_precise!(nb.report)
            end
        end
        ...
        pending = lock(nb.lock) do ;
            return ReportEngine.pending_macro_cells(nb.report)
        end
        ...
        while true
            ...
            if lock(_RUNNER_LOCK) do ;
                return get(_RUNNER_CANCEL, nb.id, false)
            end
                cancelled = true
                ReportEngine._rlog(
                    "slate: runner for $(nb.id) stopped — notebook was closed mid-drain"
                )
                break
            end
            ...
            if _parallel_enabled(nb) && lock(_eval_mutex(nb)) do ;
                return _run_code_batch!(nb)
            end
                continue
            end
            target, pending = lock(nb.lock) do
                t = _next_stale_cell(nb.report)
                return t, count(c -> c.state in (STALE, RUNNING), nb.report.cells)
            end
            target === nothing && break
            _emit_pending(nb, pending)          # k/N pill: PENDING (stale+running); frontend adds done
            lock(_eval_mutex(nb)) do ;
                return _eval_one!(nb, target)
            end
        end
        ...
        lock(_RUNNER_LOCK) do ;
            return delete!(_RUNNER_FAILS, nb.id)
        end   # clean drain (cells may have ERRORED, but no throw) → clear the streak
    catch e
        fails = lock(_RUNNER_LOCK) do ;
            return _RUNNER_FAILS[nb.id] = get(_RUNNER_FAILS, nb.id, 0) + 1
        end
        @warn "slate async runner error" notebook = nb.id fails = fails exception = (
            e, catch_backtrace()
        ) maxlog = 5
        ...
        if !was_cancelled
            ...
            giveup = lock(_RUNNER_LOCK) do ;
                return get(_RUNNER_FAILS, nb.id, 0) >= 20
            end
            if again && !giveup
                _ensure_runner!(nb)
            elseif again && giveup
                ReportEngine._rlog(
                    "slate: notebook $(nb.id) runner gave up after 20 failed passes — edit or re-run a cell to retry",
                )
            end
        end
    end
...
function _eval!(nb::LiveNotebook; wait_for::AbstractString="", wait_all::Bool=false)
    ...
    p = lock(nb.lock) do ;
        return count(c -> c.state in (STALE, RUNNING), nb.report.cells)
    end
...
function server_src_changed(nb::LiveNotebook, names::Vector{String}, err::AbstractString="")
    get(nb.report.meta, "hotreload", true) == false && return nothing
    ...
    _reads_changed(c) = any(c.reads) do r
        ...
        s = string(r)
        i = findlast('.', s)
        return i !== nothing && Symbol(SubString(s, nextind(s, i))) in syms
    end
    staled = Set{String}()
    lock(nb.lock) do
        seed = String[]
...
undo_label(nb::LiveNotebook) = lock(_LBL_LOCK) do
    s = _lblstack(_UNDO_LBL, nb)
    return isempty(s) ? "" : last(s)
end
redo_label(nb::LiveNotebook) = lock(_LBL_LOCK) do
    s = _lblstack(_REDO_LBL, nb)
    return isempty(s) ? "" : last(s)
end
...
function _snapshot!(nb::LiveNotebook; label::AbstractString="change")
    ...
    lock(_LBL_LOCK) do
        ...
        if length(nb.undo) > 100
            popfirst!(nb.undo)
            ul = _lblstack(_UNDO_LBL, nb)
            isempty(ul) || popfirst!(ul)
        end
        empty!(nb.redo)
        return empty!(_lblstack(_REDO_LBL, nb))
    end
end
...
function _restore!(nb::LiveNotebook, src::AbstractString)
    ...
    _eval!(nb)
    return _persist!(nb; source="restore")
end
...
function launch_notebook!(nb::LiveNotebook)
    hasbundle = try
        _has_bundle(read(nb.path, String))
    catch
        false
    end
    ...
    end   # flip the pill + show the banner at once
...
function _await_http_ready(url::AbstractString; timeout::Real=10)
    ...
    while time() - t0 < timeout
        try
            r = HTTP.get(
                url; retry=false, redirect=false, status_exception=false, request_timeout=2
            )
...
function _print_ready_banner(
    url::AbstractString; logpath::AbstractString="", keys::Bool=false, inactive::Bool=false
)
    rule = "─"^72
    printstyled("\n", rule, "\n"; color=:green)
    printstyled(
        if inactive
            "  ✓  Your Kaimon Slate notebook is ready (inactive)\n\n"
        else
    ...
    if keys
        ...
        print(" browser + launch (go live)    ")
        printstyled("p"; color=:cyan, bold=true)
        print(" browser, stay a preview    ")
        printstyled("q"; color=:cyan, bold=true)
...
"""
function serve_notebook(
    path::AbstractString; host="127.0.0.1", port=8765, quiet::Bool=true, inactive::Bool=false
)
    ...
    _print_ready_banner(
        url; logpath=logio === nothing ? "" : logpath, keys=showkeys, inactive=inactive
    )
    ...
    cleanup = function ()
        ...
        end         # server + SSE + every worker — nothing left to respawn
        ...
        prevlogger === nothing || (
            try
                Logging.global_logger(prevlogger)
            catch
            end
        )
        logio === nothing || (
...
function _hard_exit(code::Integer)
...
function _wait_for_ctrl_c(; on_key=nothing)
    ...
    try
        while true
            ...
            )   # other keys (b/p) act without ending the wait
        end
    finally
        close(keepraw)
        try

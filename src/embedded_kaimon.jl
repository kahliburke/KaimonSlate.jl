...
function _embedded_cache_home()
    ...
    # Per USER, because /tmp is shared: a fixed name belongs to whoever creates it first and is
    # unwritable by anyone else. `_slate_user_tag` is the same tag the worker log and memo
    # directories use, and it is one sanitised path component, which the length budget needs.
    alt = joinpath("/tmp", "ks-ai-" * ReportEngine._slate_user_tag())

...
x-axis. Use `yAxisIndex` to zoom values. Slate: `dataZoom=[(type=:inside, yAxisIndex=0)]`.""",
    ...
    EchartsDoc(
        "visualMap.type",
        """`"continuous"` (a gradient bar, default) or `"piecewise"` (discrete
...
`visualMap=(type=:piecewise, pieces=[(lt=10,), (gte=10,)])`.""",
    ...
    EchartsDoc(
        "legend",
        """Series-name legend. Slate auto-adds one when ≥2 series are `name`d; pass a
spec to place it (`legend=(orient="vertical", left="right")`) or `legend=false` to drop it,
`legend=true` to force it.""",
    ),
    EchartsDoc(
        "legend.orient",
        """`"horizontal"` (default) or `"vertical"`. Pair with `left/top/right`.
Slate: `echart(s1, s2; legend=(orient="vertical", right=8, top="middle"))`.""",
    ),
    EchartsDoc(
        "legend.selectedMode",
        """`true` (toggle any), `"single"` (one at a time), or `false`
(no toggling). Slate: `legend=(selectedMode="single",)`.""",
    ),
    EchartsDoc(
        "tooltip.trigger",
        """`"axis"` (shared crosshair — best for line/bar), `"item"` (per
...
`echart(:line, x, y; tooltip=(trigger="axis",))`, or `tooltip=false` to disable.""",
    ...
    EchartsDoc(
        "tooltip.formatter",
        """Tooltip template `"{b}: {c}"` (`{b}`=name, `{c}`=value,
`{a}`=series) or a JS function string. Slate: `tooltip=(formatter="{b}: {c} kg",)`.""",
    ),
    EchartsDoc(
        "tooltip.axisPointer.type",
        """The indicator drawn with `trigger:"axis"`: `"line"`,
`"shadow"` (bar band), `"cross"` (both axes), or `"none"`. Slate:
`tooltip=(trigger="axis", axisPointer=(type="cross",))`.""",
    ),
    EchartsDoc(
        "title.text",
        """Chart title (with optional `subtext`, `left`, `textStyle`). Slate's
`title="…"` kwarg is shorthand; full control via `title=(text="T", subtext="s", left="center")`.""",
    ...
    EchartsDoc(
        "toolbox.feature",
        """Built-in tool buttons: `saveAsImage`, `dataZoom`, `restore`,
...
a link may also be `src => tgt => val`. Style via `nodeGap`, `nodeWidth`, `lineStyle=(color="gradient",)`.""",
    ...
    EchartsDoc(
        "series.graph",
        """Network/graph: nodes + edges, force-directed + roamable by default. Slate:
...
may be `src => tgt`. Override `layout="circular"|"none"`, `force=(repulsion=…, edgeLength=…)`, node `symbolSize`.""",
    ...
    EchartsDoc(
        "series.treemap",
        """Treemap hierarchy. Slate: `echart(:treemap, tree)` where a node is
...
`:sunburst` takes the SAME data shape (radial hierarchy). Style via `levels`, `visibleMin`, `leafDepth`.""",
    ...
    EchartsDoc(
        "series.lines.geo",
        """Geo trajectories/flows: `type="lines"`, `coordinateSystem="geo"`, each
...
— coords are `(lon,lat)`. Add `effect=(show=true, symbol="arrow")` for animated flow; `lineStyle=(curveness=0.2,)`.""",
    ...
    EchartsDoc(
        "series.calendar",
        """Calendar heatmap: a heatmap series on `coordinateSystem="calendar"`, each
...
— dates are `Date`s or `"YYYY-MM-DD"`; the year range is auto-spanned. Style the `calendar=(cellSize=…, …)`.""",
    ...
    EchartsDoc(
        "color",
        """The series colour cycle: a list of colours applied in order. Slate:
`echart(:line, x, y; color=["#56d364","#f78166"])`.""",
    ),
    EchartsDoc(
        "backgroundColor",
        """Canvas background. Slate defaults it TRANSPARENT (dark theme);
set a colour with `echart(:line, x, y; backgroundColor="#0d1117")` or opt out via `theme=false`.""",
    ),
    EchartsDoc(
        "animationDurationUpdate",
        """Transition (ms) when a chart re-`setOption`s on a reactive
...
function echarts_doc_records()
    return [
        Dict{String,Any}("module" => "ECharts", "name" => e.path, "doc" => e.doc) for
        e in ECHARTS_OPTION_DOCS
    ]
end

...
function _resolve_palette(theme::AbstractString)
    return get(SLATE_PALETTES, isempty(theme) ? "midnight" : theme, SLATE_PALETTES["midnight"])
end
...
function slate_series_cycle(p::NamedTuple=SLATE_PALETTE)
    return [p.accent, p.green, p.orange, p.purple, p.teal, p.gold, p.red]
end
...
function slate_theme_attrs(p::NamedTuple=SLATE_PALETTE)
    return (
        ...
        Axis=(
            backgroundcolor=:transparent,
            xgridcolor=(p.border, 0.5),
            ygridcolor=(p.border, 0.5),
            xgridwidth=0.8,
            ygridwidth=0.8,
            topspinevisible=false,
            rightspinevisible=false,
            bottomspinecolor=p.border,
            leftspinecolor=p.border,
            xtickcolor=p.dim,
            ytickcolor=p.dim,
            xticklabelcolor=p.dim,
            yticklabelcolor=p.dim,
            xlabelcolor=p.text,
            ylabelcolor=p.text,
            titlecolor=p.text,
            titlesize=16,
        ...
        Legend=(
            framecolor=p.border, backgroundcolor=:transparent, labelcolor=p.text, titlecolor=p.dim
        ),
...
function _loaded_makie()
    for m in values(Base.loaded_modules)
        try
            nameof(m) === :Makie && return m
        catch
        end
    end
    return nothing
end
function _need_makie()
    return (
        m=_loaded_makie();
        m === nothing && error(
            "slate_theme() needs Makie loaded — add `using CairoMakie` (or GLMakie/WGLMakie) first.",
        );
        m
    )
end
...
```
"""
function slate_theme(; theme::AbstractString="")
    return Base.invokelatest(_need_makie().Theme; slate_theme_attrs(_resolve_palette(theme))...)
end
...
function use_slate_theme!(; theme::AbstractString="")
    Mk = _need_makie()
    Base.invokelatest(
        Mk.set_theme!, Base.invokelatest(Mk.Theme; slate_theme_attrs(_resolve_palette(theme))...)
    )
    return nothing
end

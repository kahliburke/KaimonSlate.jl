...
function _table_wire(t::SlatePagedTable)
...
function _paged_columns_data(x)
    ...
    if T !== nothing
        ok = try
            Base.invokelatest(T.istable, x) === true
        catch
            false
        end
        if ok
    ...
    elseif x isa NamedTuple && !isempty(x) && all(v -> v isa AbstractVector, values(x))
        return (
            String[string(k) for k in keys(x)], Vector{Any}[Any[v for v in c] for c in values(x)]
        )
...
function _like_arg(s)
    return "%" *
           lowercase(replace(string(s), "'" => "''", "\\" => "\\\\", "%" => "\\%", "_" => "\\_")) *
           "%"
end
...
function _sql_where(p::SqlPagedProvider, req::PageRequest)
    ...
    conds = [
        "LOWER(CAST($(_sql_ident(c.name)) AS VARCHAR)) LIKE '$arg' ESCAPE '\\'" for c in p.columns
    ]
...
function fetch_page(p::SqlPagedProvider, req::PageRequest)
    ...
    cnt = Base.invokelatest(
        T.rowtable, Base.invokelatest(DB.execute, p.conn, "SELECT COUNT(*) AS n " * base)
    )
    total = isempty(cnt) ? 0 : Int(cnt[1].n)
    order = ""
    if req.sort_col > 0 && req.sort_col <= length(p.columns)
        order =
            " ORDER BY $(_sql_ident(p.columns[req.sort_col].name)) " *
            (req.sort_desc ? "DESC" : "ASC")
    end
    off = (req.page - 1) * req.page_size
    cur = Base.invokelatest(
        DB.execute, p.conn, "SELECT * " * base * order * " LIMIT $(req.page_size) OFFSET $off"
    )
...
function _sql_provider(conn, sql::AbstractString)
    DB = _dbinterface_mod()
    DB === nothing && throw(
        ArgumentError("slate_query requires DBInterface.jl + a driver (DuckDB/SQLite) loaded")
    )
    T = _tables_mod()
    ...
    types = if sch.types === nothing
        fill(:string, length(names))
    else
        Symbol[_sql_coltype(t) for t in sch.types]
    end
    cols = ColumnDef[
        ColumnDef(names[i], types[i], _default_align(types[i]), nothing, true, true) for
        i in eachindex(names)
    ]
...
"""
function slate_query(conn, sql::AbstractString; page_size::Int=50)
    return _make_paged(_sql_provider(conn, sql); page_size=page_size)
end

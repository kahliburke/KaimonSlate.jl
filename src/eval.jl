...
export register_refresh!,
    unregister_refresh!, register_srcchange!, unregister_srcchange!, revise_apply!
...
function register_refresh!(report_id::AbstractString, cb)
    return (_REFRESH_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_refresh!(report_id::AbstractString)
    return (delete!(_REFRESH_REGISTRY, String(report_id)); nothing)
end
...
function register_progress!(report_id::AbstractString, cb)
    return (_PROGRESS_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_progress!(report_id::AbstractString)
    return (delete!(_PROGRESS_REGISTRY, String(report_id)); nothing)
end
function _emit_progress(report_id::AbstractString, cell)
    ...
    try
        ...
        @debug "eval: progress callback failed" report_id exception = e
    end   # best-effort — a push failure must never break eval
    return nothing
end
...
function register_runbatch!(report_id::AbstractString, cb)
    return (_RUNBATCH_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_runbatch!(report_id::AbstractString)
    return (delete!(_RUNBATCH_REGISTRY, String(report_id)); nothing)
end
function _emit_run_batch(report_id::AbstractString, n::Integer)
    ...
    try
        ...
        @debug "eval: run-batch callback failed" report_id exception = e
    end
    return nothing
end
...
function register_userprog!(report_id::AbstractString, cb)
    return (_USERPROG_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_userprog!(report_id::AbstractString)
    return (delete!(_USERPROG_REGISTRY, String(report_id)); nothing)
end
function _do_userprog(report_id::AbstractString, frac, msg, id="", done=false)
    cb = get(_USERPROG_REGISTRY, String(report_id), nothing)
    cb === nothing && return nothing
    f = try
        clamp(Float64(frac), 0.0, 1.0)
    catch
        0.0
    end
    try
        cb(f, String(msg), String(id), done === true)
    catch e
        @debug "eval: user-progress callback failed" report_id exception = e
    end
    return nothing
end
...
function register_prepare!(report_id::AbstractString, cb)
    return (_PREPARE_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_prepare!(report_id::AbstractString)
    return (delete!(_PREPARE_REGISTRY, String(report_id)); nothing)
end
function _do_prepare(report_id::AbstractString, json::AbstractString)
    ...
    try
        ...
        @debug "eval: prepare callback failed" report_id exception = e
    end
    return nothing
end
...
function _do_emit(report_id::AbstractString, channel, payload)
    ...
    cb === nothing && return nothing
    try
        cb(String(channel), payload)
    catch e
        @debug "eval: emit callback failed" report_id exception = e
    end   # payload is a Julia VALUE (gate: deserialized; in-process: passed straight through) — the emit callback JSON-encodes it
    return nothing
end
...
function register_bin_emit!(report_id::AbstractString, cb)
    return (_BIN_EMIT_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_bin_emit!(report_id::AbstractString)
    return (delete!(_BIN_EMIT_REGISTRY, String(report_id)); nothing)
end
function _do_emit_bin(report_id::AbstractString, frame::Vector{UInt8})
    ...
    try
        ...
        @debug "eval: bin emit callback failed" report_id exception = e
    end
    return nothing
end
...
function register_celldone!(report_id::AbstractString, cb)
    return (_CELLDONE_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_celldone!(report_id::AbstractString)
    return (delete!(_CELLDONE_REGISTRY, String(report_id)); nothing)
end
function _do_celldone(report_id::AbstractString, run_id, cell_id, wire)
    cb = get(_CELLDONE_REGISTRY, String(report_id), nothing)
    cb === nothing && return nothing
    try
        cb(String(run_id), String(cell_id), wire)
    catch e
        @warn "slate: celldone merge failed" cell = cell_id exception = e
    end
    return nothing
end
...
function register_cleanup_cells!(report_id::AbstractString, cb)
    return (_CLEANUP_CELLS_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_cleanup_cells!(report_id::AbstractString)
    return (delete!(_CLEANUP_CELLS_REGISTRY, String(report_id)); nothing)
end
function _do_cleanup_cells(report_id::AbstractString, ids)
    ...
    try
        ...
        @warn "slate: cell-delete cleanup failed" ids = sids exception = e
    end
    return nothing
end
...
function register_srcchange!(report_id::AbstractString, cb)
    return (_SRCCHANGE_REGISTRY[String(report_id)]=cb; nothing)
end
function unregister_srcchange!(report_id::AbstractString)
    return (delete!(_SRCCHANGE_REGISTRY, String(report_id)); nothing)
end
...
function _new_module(report::Report)
    ...
    _populate_notebook_ns!(
        ...
        slate_refresh=(vars...) -> _do_refresh(rid, vars),
        ...
        slate_emit=(channel, data) -> if data isa SlateExtensionsBase.SlateBinary
            _do_emit_bin(rid, SlateExtensionsBase.encode_binary_frame(string(channel), data))
        else
            _do_emit(rid, channel, data)
        end,
        assetbase=() -> String(get(report.meta, "assetbase", "")),
    )   # `@asset` base (notebook project dir)
    return m
end
...
function _eval_capture(
    mod::Module, source::AbstractString, filename::AbstractString="string"; slate_ctx=nothing
)
    ...
    return CellOutput(
        ...
        "",
        effects,
        assets,   # in-process has no memo; effects/assets from their channels
...
function run_cleanups!(::InProcessKernel, report::Report, ids)
    ...
    reg =
        isdefined(report.mod, :__slate_cleanups) ? getfield(report.mod, :__slate_cleanups) : nothing
...
function eval_capture(
    ...
    return eval_capture(_await_real(k), report, source, filename; region=region, regions=regions)
end
function eval_capture(
    k::PendingKernel,
    report::Report,
    ...
    return eval_capture(
        _await_real(k), report, source, filename, memo; region=region, regions=regions
    )
end
function complete(k::PendingKernel, report::Report, code::AbstractString, pos::Integer)
    return complete(_await_real(k), report, code, pos)
end
function assign_bind!(k::PendingKernel, report::Report, name::Symbol, value)
    return assign_bind!(_await_real(k), report, name, value)
end
function table_page(
    k::PendingKernel, report::Report, table_id::AbstractString, request::AbstractDict
)
    return table_page(_await_real(k), report, table_id, request)
end
function interpolate(k::PendingKernel, report::Report, exprs::Vector{String})
    return interpolate(_await_real(k), report, exprs)
end
function harvest_docs(k::PendingKernel, report::Report, mod_names)
    return harvest_docs(_await_real(k), report, mod_names)
end
function module_help(k::PendingKernel, report::Report, name::AbstractString)
    return module_help(_await_real(k), report, name)
end
function macroexpand_cells(k::PendingKernel, report::Report, srcs::AbstractDict)
    return macroexpand_cells(_await_real(k), report, srcs)
end
...
function pkg_op(
    ...
    return pkg_op(_await_real(k), report, op, name; target)
end
...
"""
function complete(::InProcessKernel, report::Report, code::AbstractString, pos::Integer)
    return slate_completions(report_module(report), code, pos)
end
...
function assign_bind!(::InProcessKernel, report::Report, name::Symbol, value)
    ...
    return (
        ...
        Base.invokelatest(() -> getfield(m, :__slate_set_bind)(name, value))
    )
end
...
"""
function table_page(::InProcessKernel, ::Report, table_id::AbstractString, request::AbstractDict)
    return _provider_page(table_id, _page_request(request))
end
...
"""
function interpolate(::InProcessKernel, report::Report, exprs::Vector{String})
    return CellOutput[_eval_capture(report_module(report), e) for e in exprs]
end
...
"""
function harvest_docs(::InProcessKernel, report::Report, mod_names)
    return harvest_module_docs(report_module(report), mod_names)
end
...
"""
function module_help(::InProcessKernel, report::Report, name::AbstractString)
    return module_help(report_module(report), name)
end
...
function _dep_closure(byid, cell::Cell)
    closure = Set{String}()
    stack = collect(cell.deps)
    while !isempty(stack)
        id = pop!(stack)
        (id in closure || !haskey(byid, id)) && continue
        push!(closure, id)
        union!(stack, byid[id].deps)
    end
    return closure
end
...
function _memo_key(report::Report, cell::Cell)
    ...
    for id in closure
        st = _frozen_stamp(byid[id])
        isempty(st) || push!(frozen, (id, st))
    end
    sort!(frozen)
    readnames = copy(cell.reads)
    for id in closure
        union!(readnames, byid[id].reads)
    end
...
function _memo_status(report::Report, cell::Cell)
    ...
    :opaque in cell.flags &&
        return ("uncacheable", "an unresolved `using`/macro barrier — its effects can't be tracked")
    eff = _cell_effect(cell)
    eff == VOLATILE && return (
        "uncacheable", "non-deterministic (e.g. `rand`/`time`) — a cached value would be stale"
    )
    eff == RESOURCE && return (
        "uncacheable",
        "opens a live handle (DB/socket/file) — tag it `resource`; it re-opens each run",
    )

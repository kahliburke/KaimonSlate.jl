...
function _fp!(ctx, x)
    ...
    elseif x isa Bool
        _fp_bytes!(ctx, 0x02)
        _fp_bytes!(ctx, UInt8(x))
    elseif x isa BigInt
        _fp_bytes!(ctx, 0x03)
        _fp_bytes!(ctx, codeunits(string(x)))
    elseif x isa Integer && typemin(Int64) <= x <= typemax(Int64)
        _fp_bytes!(ctx, 0x04)
        _fp_u64!(ctx, reinterpret(UInt64, Int64(x)))   # widen: Int32(3) ≡ 3
    elseif x isa AbstractFloat
        b = reinterpret(UInt64, Float64(x))
        _fp_bytes!(ctx, 0x05)
        _fp_u64!(ctx, isnan(x) ? _FP_CANON_NAN : b)    # NaN ≡ NaN; -0.0 ≢ 0.0
    elseif x isa AbstractString
        _fp_bytes!(ctx, 0x06)
        _fp_u64!(ctx, UInt64(ncodeunits(x)))
        _fp_bytes!(ctx, codeunits(String(x)))
    elseif x isa Symbol
        _fp_bytes!(ctx, 0x07)
        _fp_bytes!(ctx, codeunits(String(x)))
    elseif x isa Char
        _fp_bytes!(ctx, 0x08)
        _fp_u64!(ctx, UInt64(codepoint(x)))
    elseif x isa Tuple
        _fp_bytes!(ctx, 0x09)
        _fp_u64!(ctx, UInt64(length(x)))
        for v in x
            _fp!(ctx, v)
        end
    ...
    elseif x isa AbstractArray
        _fp_bytes!(ctx, 0x0e)
        _fp_u64!(ctx, UInt64(ndims(x)))
        for d in size(x)
            _fp_u64!(ctx, UInt64(d))
        end
        for v in x
            _fp!(ctx, v)
        end
        ...
        bs = try
            io = IOBuffer()
            _FPSerialization.serialize(io, x)
            take!(io)
        catch
            Vector{UInt8}(codeunits(repr(x)))
        end
        _fp_u64!(ctx, UInt64(length(bs)))
        _fp_bytes!(ctx, bs)
    end
    return nothing
end
...
function _memostore_ctx()
    return if (
        isdefined(Main, :SlateWorker) &&
        isdefined(Main.SlateWorker, :MemoStore) &&
        Main.SlateWorker._MEMO_OK
    )
        (Main.SlateWorker.MemoStore, Main.SlateWorker._memo_dir())
    else
        nothing
    end
end
...
function slate_memo_entries(; name::AbstractString="")
    ...
    for f in sort!(filter(isfile, readdir(mdir; join=true)); by=mtime, rev=true)
        ...
        elided = join(
            (String(get(e, "name", "")) for e in get(d, "elided", Any[]) if e isa AbstractDict),
            ", ",
        )
        push!(
            out,
            (
                key=replace(basename(f), r"\.toml$" => ""),
                names=join(nms, ", "),
                bytes=bytes,

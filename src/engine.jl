...
function parse_report(text::AbstractString; id::AbstractString = "r", title::AbstractString = "")
    ...
    lines = split(text, '\n')
    # Split off the reproducibility footer (always terminal) before cell parsing, so its
    # comment lines are never mistaken for a Literate markdown cell.
    fi = findfirst(l -> startswith(l, _ENV_MARK_OPEN), lines)
    if fi !== nothing
        env = _parse_env_footer(@view lines[fi:end])
...
function serialize_report(report::Report)
    body = join((_cell_source(c) for c in report.cells), "\n\n") * "\n"

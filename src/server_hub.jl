    ...
    rows = [
        with_report(nb) do report
            ...
            return Dict(
                ...
                "compute_ms" =>
                    sum(c -> c.output === nothing ? 0.0 : c.output.duration_ms, cs; init=0.0),
                "mtime" => _file_mtime(nb.path),
                "worker" => _worker_label(nb),
                "port" => _worker_port(nb),
...
function _worker_label(nb::LiveNotebook)
    return if nb.kernel isa GateKernel && nb.kernel.port != 0
        " · worker&nbsp;:$(nb.kernel.port)"
    else
        ""
    end
end
...
function _worker_port(nb::LiveNotebook)
    return nb.kernel isa GateKernel && nb.kernel.port != 0 ? nb.kernel.port : nothing
end

# Where this notebook's worker actually runs — "" (local) or the ssh host — for the index card badge.
function _worker_location(nb::LiveNotebook)
    return if (nb.kernel isa GateKernel && nb.kernel.target isa ReportEngine.RemoteTarget)
...
_file_mtime(path::AbstractString) =
    try
        round(Int, mtime(abspath(path)))
    catch
        0
    end
...
function _html(body)
    return HTTP.Response(
        200, ["Content-Type" => "text/html", "Cache-Control" => "no-store", _FRAME_GUARD...], body
    )
end
function _asset(body, ctype)
    return HTTP.Response(
        200, ["Content-Type" => ctype, "Cache-Control" => "no-store", _FRAME_GUARD...], body
    )
end
...
const _VENDOR_CACHE = get(
    ENV,
    "KAIMONSLATE_ASSET_CACHE",
    joinpath(
        get(DEPOT_PATH, 1, joinpath(homedir(), ".julia")), "scratchspaces", "kaimonslate-assets"
    ),
)
...
function _vendor_file(pkg::AbstractString, sub::AbstractString)
    ...
    base = replace(String(entry["base"]), "{version}" => ver)
...
function _persist_registry!(h::Hub)
    data = lock(h.lock) do
        return Dict{String,String}(id => abspath(nb.path) for (id, nb) in h.notebooks)
    end
    try
        f = _registry_file(h)
        mkpath(dirname(f))
...
function _reopen_persisted!(h::Hub, id::AbstractString)
    ...
    return lock(_REOPEN_LOCK) do
        ...
        nb = try
            ...
            @warn "Kaimon Slate: failed to re-open notebook after restart" id path exception =
                err
            return nothing
        end
        lock(h.lock) do ;
            return h.notebooks[id] = nb
        end
        _start_watcher!(nb)
        _persist_registry!(h)
...
function _path_completions(q::AbstractString; limit::Int=500)
    ...
    sort!(
        keep;
        by=p -> (
            (endswith(p, '/') || endswith(p, '\\')) ? 0 : (endswith(p, ".jl") ? 1 : 2),
            lowercase(p),
        ),
    )

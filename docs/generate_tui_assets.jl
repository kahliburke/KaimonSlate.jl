...
function _find_font()
    ...
    return isempty(fonts) ? "" : fonts[1].path
end
...
const EVENTS = EventScript(
    pause(1.6), rep(key(:down), 2; gap=0.7), pause(0.9), rep(key(:up), 2; gap=0.6), pause(1.4)
)
...
function main()
    ...
    try
        ...
        Base.invokelatest(
            export_gif_from_snapshots,
            joinpath(ASSETS_DIR, "slate-tui.gif"),
            tw,
            th,
            cells,
            ts;
    ...
    return println("done — $(ASSETS_DIR)")
end

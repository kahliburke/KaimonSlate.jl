...
function _mknb(src)
    return NS.LiveNotebook(
        "fm",
        "/tmp/fmtest.jl",
        RE.parse_report(src; title="Fallback"),
        RE.InProcessKernel(),
        1,
        String[],
        String[],
...
@testset "front-matter roles" begin
    ...
    @testset "HTML export: citations rendered + References + echarts embedded client-side" begin
        ...
        @test m !== nothing &&
            occursin("Building on 1", m.captures[1]) &&
            !occursin("knuth", m.captures[1])
        ...
        ec.output = RE.CellOutput(
            "",
            RE.MimeChunk[],
            Any[Dict("series" => [Dict("type" => "bar", "data" => [1, 2, 3])])],
            Any[],
            RE.BindSpec[],
            "",
            nothing,
    ...
    @testset "Markdown export (copy-to-clipboard)" begin
        ...
        @test occursin("```julia\nx = 40 + 2\n```", m2)      # code fenced
        @test !occursin(
            "```julia", NS.export_markdown(_mknb("#%% code id=c\nx = 1\n"); include_source=false)
        )
        # GFM table helper
        ...
        cf.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[1])],
            Any[],
            Any[],
            RE.BindSpec[],
            "",
    ...
    @testset "figure captions: numbering + binding (flow + explicit)" begin
        ...
        @test occursin("for=scaling", RE.serialize_cells(r)) &&
            occursin("label=fig:scale", RE.serialize_cells(r))
        ...
        for id in ("f1", "f2")
            ...
            fc.output = RE.CellOutput(
                "", [img], Any[], Any[], RE.BindSpec[], "", nothing, nothing, 0.0
            )
        end
    ...
    @testset "figure cross-references [@fig:label]" begin
        ...
        live = NS._rewrite_citations(
            "See [@fig:hist] above."; figrefs=figrefs, figemit=NS._fig_link_emit
        )
        @test occursin("<a class=\"figref\" href=\"#cell-hist\">Figure 2</a>", live)
        # bare form works too
        @test occursin(
            "Figure 2",
            NS._rewrite_citations("as in @fig:hist"; figrefs=figrefs, figemit=NS._fig_link_emit),
        )
        ...
        @test occursin(
            "[@knuth]", NS._rewrite_citations("[@knuth]"; figrefs=figrefs, emit=NS._cite_literal)
        )
        # a literal in backticks is left alone
        @test occursin(
            "`[@fig:hist]`",
            NS._rewrite_citations("`[@fig:hist]`"; figrefs=figrefs, figemit=NS._fig_link_emit),
        )
    end

    @testset "outputs filter (all / figures / none)" begin
        @test NS._outputs_text_ok("all") &&
            !NS._outputs_text_ok("figures") &&
            !NS._outputs_text_ok("none")
        ...
        fc.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x89])],
            Any[],
            Any[],
            RE.BindSpec[],
            "42",
    ...
    @testset "site export: og:image meta + tarball" begin
        ...
        fc.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x89, 0x50])],
            Any[],
            Any[],
            RE.BindSpec[],
            "",
        ...
        vc.output = RE.CellOutput(
            "", RE.MimeChunk[], Any[], Any[], RE.BindSpec[], "(a = 1,)", nothing, nothing, 0.0
        )
        ff.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x89, 0x50])],
            Any[],
            Any[],
        ...
        t1.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x01])],
            Any[],
            Any[],
            RE.BindSpec[],
            "",
        ...
        t2.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x02])],
            Any[],
            Any[],
            RE.BindSpec[],
            "",
        ...
        ob.output = RE.CellOutput(
            "",
            [RE.MimeChunk("image/png", UInt8[0x03])],
            Any[],
            Any[],
            RE.BindSpec[],
            "",
        ...
        @test occursin("<button id=\"exp-run-btn\">", rh) &&
            occursin("Run this notebook live", rh) &&
            occursin("run.jl", rh)
        @test !occursin("<button id=\"exp-run-btn\">", NS.export_html(fnb))   # only when runnable (CSS is always present)
        rj = NS._run_script(
            "https://x.github.io/y/nb.standalone.jl"; agent=true, bundle_name="nb.standalone.jl"
        )
    ...
    @testset "multi-document site (blog)" begin
        ...
        d = mktempdir()
        entry = NS._build_doc!(
            joinpath(d, "predicting-foo-from-bar"), nb; slug="predicting-foo-from-bar"
        )
        @test isfile(joinpath(d, "predicting-foo-from-bar", "index.html"))
        @test isfile(joinpath(d, "predicting-foo-from-bar", "og-image.png"))
        @test entry["slug"] == "predicting-foo-from-bar" &&
            entry["title"] == "Predicting Foo from Bar"
        ...
        @test entry["cells"] == length(nb.report.cells) &&
            entry["code"] + entry["md"] == entry["cells"]
        @test haskey(entry, "updated") &&
            haskey(entry, "binds") &&
            entry["packages"] isa AbstractVector
        ...
        write(
            joinpath(d, NS._SITE_MANIFEST),
            "{\"title\":\"T\",\"docs\":[{\"slug\":\"x\",\"title\":\"X\"}]}",
        )
    ...
    @testset "build_site! merges multiple docs into one dir (no git)" begin
        ...
        withfig(nb) = (
            nb.report.cells[end].output=RE.CellOutput(
                "",
                [RE.MimeChunk("image/png", UInt8[0x89, 0x50])],
                Any[],
                Any[],
        ...
        @test isfile(joinpath(d, "alpha", "index.html")) &&
            isfile(joinpath(d, "beta", "index.html"))
    ...
    @testset "export_to_site + _site_file traversal guard" begin
        ...
        withenv("KAIMONSLATE_SITES_DIR" => tmp) do
            ...
            @test isdir(joinpath(tmp, "my-portfolio", "alpha")) &&
                "my-portfolio" in NS.list_local_sites()
            ...
            @test NS._site_file("my-portfolio", "alpha") ==
                joinpath(tmp, "my-portfolio", "alpha", "index.html")
            ...
            @test Set(String(get(d, "slug", "")) for d in NS.site_docs("My Portfolio")) ==
                Set(["alpha", "beta"])
    ...
    @testset "front page (home tag + docindex placeholder)" begin
        ...
        @test occursin("id=\"slate-cards-root\"", filled) &&
            occursin("fetch('slate-site.json'", filled)  # + client refresh
    ...
    @testset "bibliography index + per-cell card info" begin
        ...
        @test NS._delatex(raw"The Pseudoprimes to $25 \cdot 10^9$") ==
    ...
    @testset "citation rewriting (locators / forms / safety)" begin
        ...
        @test rw("show `[@knuth1984]` then [@knuth1984]") ==
            "show `[@knuth1984]` then §c§n§knuth1984§§"
        ...
        numctx = (
            anchor="refs",
            tips=Dict("k" => "Knuth · TeX"),
            labels=Dict("k" => "3"),
            numeric=true,
            numbers=Dict("k" => 3),
        )
        num = NS._rewrite_citations("[@k, p. 7]", Set(["k"]); emit=NS._cite_link_emit(numctx))
        @test occursin("<a class=\"cite\" href=\"#cell-refs\"", num) &&
            occursin(">[3, p. 7]</a>", num)
        adctx = (
            anchor="refs",
            tips=Dict("k" => "x"),
            labels=Dict("k" => "Knuth, 1984"),
            numeric=false,
    ...
    @testset "adaptive references card + mixed inline/external bib" begin
        ...
        io = IOBuffer()
        for i in 1:12
            println(io, "@book{ext$i, title={T$i}}")
        end
        ...
        @test occursin("li class=\"cited\"", inl["output"]) &&
            occursin("li class=\"uncited\"", inl["output"])  # small → all, highlighted
        rm(d; recursive=true, force=true)
    end

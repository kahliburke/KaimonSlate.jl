...
@testset "ssh mux ControlPath stays inside sun_path" begin
    ...
    # Win32-OpenSSH has no multiplexing: ControlMaster/ControlPath make ssh exit with
    # "getsockname failed: Not a socket" before running the command, so every remote call
    # fails on Windows while plain ssh to the same host works.
    if Sys.iswindows()
        @test isempty(RE._ssh_mux_opts())
        @test isempty(RE._rsync_ssh_opt())
    end
end

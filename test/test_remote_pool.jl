...
function mkworker(
    ...
    hub=gethostname(),
    transport="tunnel",
    project="~/.cache/kaimonslate/remote/examples",
    stream_port=port + 1,
)
    return Dict{String,Any}(
        ...
        "stats" => "",
        "manifest" =>
            "{\"notebook\":\"\",\"region\":\"$region\",\"hub\":\"$hub\",\"transport\":\"$transport\"," *
...
@testset "remote pool + park bookkeeping" begin
    ...
    @testset "region registry: round-trip, listing, target keyed by preload basename" begin
        ...
        withenv("KAIMONSLATE_CONFIG_HOME" => mktempdir()) do   # isolate regions.json from the real one
            ...
            @test any(
                x ->
                    x.name == RE._fold_region(name) &&
                    x.warm == 2 &&
                    x.data_root == "/scratch/flights",
                RE.regions(),
            )
            ...
            @test RE._region_target(RE.region_get(name)).project ==
    ...
    @testset "_region_target: origin_env override decouples the env from preload" begin
        ...
        withenv("KAIMONSLATE_CONFIG_HOME" => mktempdir()) do
            ...
            @test RE._region_target(r).project ==
                "~/.cache/kaimonslate/remote/" * RE._proj_key("/tmp/warm-proj")
    ...
    @testset "claim set: exclusive, idempotent release" begin
        ...
        lock(RE._REGION_CLAIM_LOCK) do ;
            push!(RE._REGION_CLAIMS, (h, 9100))
        end
        @test RE._region_claimed(h, 9100)
    ...
    @testset "parked wires: park on detach, unpark once, evict by port/label" begin
        ...
        withenv("KAIMONSLATE_REMOTE_LOG" => joinpath(mktempdir(), "remote.log")) do
            ...
            mkk(label, port) = (
                k=RE.GateKernel("~/proj"; label=label, target=t);
                k.conn=:fake_conn;
                k.port=port;
                k.stream_port=port + 1;
                k
            )
            # park is refused without the preconditions (no conn / no port / not remote)
            k0 = RE.GateKernel("~/proj"; label="np.jl", target=t)
            k0.port = 9100
...
""",
    ...
    @testset "_env_instantiate_script: rewrites Manifest AND Project.toml [sources]" begin
        s = RE._env_instantiate_script(
            ".cache/kaimonslate/remote/NeuroSlate",
    ...
    @testset "remote timing config: default → env → slate.json precedence" begin
        ...
        try
            ...
            withenv(
                ...
                RE._REMOTE_CFG[] = Dict{String,Any}(
                    "blob_chunk_timeout" => 45, "peer_bw_mbps" => 12
                )

...
function _clear()
    return Dict(
        ...
        "KAIMONSLATE_DATA_HOME" => nothing,
        "KAIMONSLATE_CACHE_HOME" => nothing,
        "KAIMONSLATE_SITES_DIR" => nothing,
...
@testset "SlateHome — XDG home resolution" begin
    ...
    @testset "per-home override beats KAIMONSLATE_HOME" begin
        withenv(
            _env("KAIMONSLATE_HOME" => "/tmp/ks", "KAIMONSLATE_CONFIG_HOME" => "/elsewhere/cfg")...
        ) do
    ...
    @testset "XDG vars → kaimonslate namespace subdir" begin
        withenv(
            _env("XDG_CONFIG_HOME" => "/xc", "XDG_DATA_HOME" => "/xd", "XDG_CACHE_HOME" => "/xk")...
        ) do
    ...
    @testset "KAIMONSLATE_SITES_DIR is the most-specific sites override" begin
        withenv(
            _env("KAIMONSLATE_HOME" => "/tmp/ks", "KAIMONSLATE_SITES_DIR" => "/custom/sites")...
        ) do
...
@testset "PublishLedger — record + JSON round-trip" begin
    l = PL.Ledger()
    l.targets["gh:portfolio"] = PL.Target(
        "gh:portfolio", "github-pages"; config=Dict("repo" => "u/portfolio", "branch" => "gh-pages")
    )
...
@testset "PublishLedger — union merge" begin
    a = PL.Ledger()
    PL.record_event!(a, "d", "t"; id="e1", note="first")
    a.documents["d"].title = "Original"
    b = PL.Ledger()
    PL.record_event!(b, "d", "t2"; id="e2", note="second")
    b.documents["d"].title = "Updated"
    b.documents["d"].slug = ""   # empty slug must not blank existing
...
function PL.gist_read(c::FakeGh, id, filename)
    return (
        i=findfirst(g -> g.id == id, c.gists);
        i === nothing ? nothing : get(c.gists[i].files, filename, nothing)
    )
end
...
function PL.gist_update(c::FakeGh, id, filename, content)
    i = findfirst(g -> g.id == id, c.gists)
    i === nothing && error("no gist $id")
    c.gists[i].files[String(filename)] = String(content)
    return nothing
end
...
function NS.publish(t::FakeTarget, nb; kwargs...)
    ...
    ok = t.behavior === :ok
    return NS.PublishResult(;
        ok=ok,
        url="https://x/$(t.name)",
        commit="c_$(t.name)",
        status=ok ? "ok" : "failure",
        log=ok ? "" : "failed-$(t.name)",
    )
end

@testset "PublishTargets — fan-out, isolation, events" begin
    @testset "publish_to_targets preserves order + isolates failures" begin
        targets = NS.PublishTarget[
            FakeTarget("a", :ok), FakeTarget("b", :throw), FakeTarget("c", :fail)
        ]
        seq = Tuple{Int,Symbol}[]
        results = NS.publish_to_targets(
            nothing, targets; on_event=(i, ph, _) -> push!(seq, (i, ph))
        )
    ...
    @testset "_record_results! appends one event per target" begin
        led = PL.Ledger()
        res = [
            NS.PublishResult(; ok=true, url="u1", commit="c1", status="ok"),
...
@testset "PublishTargets — ledger config → adapter" begin
    ...
    s3 = NS.target_from_ledger(
        PL.Target(
            "r2:x",
            "r2";
            config=Dict(
                "dest" => "s3://bucket/p", "endpoint" => "https://ep", "url" => "https://cdn/"
            ),
        ),
    )
    ...
    zt = NS.target_from_ledger(
        PL.Target("z", "zenodo"; config=Dict("secretRef" => "zenodo-token"));
...
function NS.zenodo_request(c::FakeZenodo, method, url; json=nothing, file=nothing)
    ...
    elseif method == "POST" && endswith(url, "/actions/newversion")
        ...
        c.newversioned = true
        return (
            201, Dict("links" => Dict("latest_draft" => "https://z/api/deposit/depositions/222"))
        )
...
@testset "Publish service — secrets, targets, view" begin
    ...
    mktempdir() do tmp
        withenv(
            _env(
                "KAIMONSLATE_LEDGER_BACKEND" => "local",
                "KAIMONSLATE_DATA_HOME" => tmp,
                "KAIMONSLATE_CONFIG_HOME" => joinpath(tmp, "cfg"),
            )...,
        ) do
            ...
            @testset "target add/delete + ledger view shape" begin
                ...
                @test Set(t["name"] for t in NS.publish_ledger_view()["targets"]) ==
                    Set(["gh:site"])
            end
        end
    end
...
@testset "Zenodo — deposition flow" begin
    mktempdir() do tmp
        file = joinpath(tmp, "nb.jl")
        write(file, "# bundle")
        meta = Dict{String,Any}(
            "upload_type" => "software", "title" => "WG", "creators" => [Dict("name" => "K")]
        )
        ...
        @testset "an HTTP error aborts cleanly (no publish)" begin
            ...
            @test !r2.ok &&
                occursin("Zenodo HTTP", r2.log) &&
                !endswith(last(c.calls)[2], "/actions/publish")
        end
    end
end
...
@testset "sites: titles, removal hygiene, purge plumbing" begin
    mktempdir() do tmp
        withenv(
            _env("KAIMONSLATE_HOME" => tmp, "KAIMONSLATE_SITES_DIR" => joinpath(tmp, "sites"))...
        ) do
            ...
            mkpath(dir)
            write(joinpath(dir, "index.html"), "x")
...
@testset "site subpaths within a target" begin
    mktempdir() do tmp
        withenv(
            ...
            NS.publish_target_set!(
                "bucket", "s3", Dict{String,Any}("dest" => "s3://b", "url" => "https://cdn/")
            )
            NS.publish_target_set!("gh", "github-pages", Dict{String,Any}("repo" => "u/r"))
            ...
            NS.publish_site_set!(
                "stat", ["bucket"], "", "Stat"; paths=Dict("bucket" => "stat-mech")
            )
            led = PL.load(PL.default_store())
            ...
            @test_throws ErrorException NS.publish_site_set!(
                "dup", ["bucket"]; paths=Dict("bucket" => "blog")
            )
            ...
            NS.publish_site_set!(
                "stat", ["bucket"], "", "Stat 2"; paths=Dict("bucket" => "stat-mech")
            )
            ...
            @test gh.subdir == "blog"
            @test_throws ErrorException NS.with_subpath(
                NS.target_from_ledger(led.targets["cf"]), "x"
            )
        end
    end
end

...
function _out(html; live::Bool)
    return RE.CellOutput(
        "",
        RE.MimeChunk[RE.MimeChunk("text/html", Vector{UInt8}(html))],
        Any[],
        Any[],
        RE.BindSpec[],
        "",
...
@testset "live (session-bound) outputs" begin
    @testset "CellOutput.live defaults false through the back-compat ladder" begin
        ...
        rungs = (
            RE.CellOutput(
                "", RE.MimeChunk[], Any[], Any[], RE.BindSpec[], "", nothing, nothing, 0.0
            ),
            RE.CellOutput(
                "",
                RE.MimeChunk[],
    ...
    @testset "the wire's `live` flag reaches CellOutput" begin
        wire = (
            stdout="",
            mime=[("text/html", Vector{UInt8}(FIG_HTML))],
            echarts=Any[],
            tables=Any[],
            binds=NamedTuple[],
            value_repr="",
